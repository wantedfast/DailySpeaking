...
import type { DocumentSource } from '@/lib/knowledge';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (session.stage === 'speech') {
      setRecords(r => addRecord(r,{id:session.id,word:session.topic!.word,category:session.category,minutes:session.minutes,source:session.source,date:new Date().toISOString()}));
  ...
  async function draw() {
    if(!session) return;
    const topic = await request<Topic>('topic',{category:session.category,documentId:session.source?.id,recentWords:words},'正在为你寻找一个值得探索的词…');
    if(topic) {
      setSession({...freshSession(session.category),source:session.source,stage:'topic',topic});
      setWords(w => [topic.word,...w.filter(word => word !== topic.word)].slice(0,30));
    }
  }
  async function research() {
    if(!session?.topic) return;
    const result = await request<Research>('research',{category:session.category,documentId:session.source?.id,word:session.topic.word},'正在整理概念、例子和理解线索，通常需要一点时间…');
  ...
  async function makeSpeech() {
    ...
    const result = await request<Speech>('speech',{word:session.topic!.word,research:{sections:session.research.sections,questions:session.research.questions},minutes:session.minutes},'正在把知识整理成自然的口头表达…');
  ...
  return <div className="site-shell">
    <header className="site-header"><a href={appUrl('/')} onClick={e => { e.preventDefault(); reset(); }} className="brand" aria-label="每日开讲首页"><span className="brand-icon"><AudioLines size={23}/></span><span>每日开讲<span className="brand-sub">DAILY SPEAKING</span></span></a><button className="quiet-button" disabled={!!busy} onClick={() => setShowKnowledge(true)}><BookOpen size={17}/><span>知识库</span></button><button className="quiet-button" onClick={() => setShowHistory(true)}><History size={17}/><span>练习足迹</span>{records.length > 0 && <span className="badge">{records.length}</span>}</button></header>
    <main>
      ...
      {session.stage === 'choose' && <>
        ...
        <section className="category-section">{session.source ? <div className="document-selection"><span className="eyebrow">FROM YOUR LIBRARY</span><h3>{session.source.name}</h3><p>从这份文档抽取主题，依据原文准备学习材料与讲解稿。</p><div className="button-row"><button className="secondary" disabled={!!busy} onClick={()=>setShowKnowledge(true)}>更换文档</button><button className="quiet-button" disabled={!!busy} onClick={()=>reset()}>改为领域练习</button></div></div> : <><div className="section-label"><h2>今天，想探索哪个领域？</h2><span>选一个方向，把剩下的交给好奇心</span></div><div className="category-grid">{categories.map(c => { const Icon=icons[c.icon]; const selected=session.category===c.id; return <button className={`category-card ${selected?'selected':''}`} aria-pressed={selected} key={c.id} onClick={() => setSession({...session,category:c.id})} disabled={!!busy}><div className="category-top"><span className="category-icon"><Icon size={26} strokeWidth={1.4}/></span><span className="radio-dot">{selected && <span/>}</span></div><div className="category-en">{c.en}</div><h3>{c.name}</h3><p>{c.desc}</p><div className="category-sample">{c.sample}</div></button>; })}</div></>}<div className="draw-area"><button className="primary draw-button" onClick={draw} disabled={!!busy}><Shuffle size={18}/> 抽取今日关键词 <ArrowRight size={18}/></button><p>不需要懂很多，愿意开始就很好。</p></div></section>
      ...
      {session.stage === 'topic' && <section className="topic-stage"><button className="back-link" disabled={!!busy} onClick={() => reset()}><ChevronLeft size={16}/> {session.source?'重新选择资料':'重新选择领域'}</button><div className="eyebrow">TODAY’S DISCOVERY · {sourceName}</div><p className="soft-label">今天，来认识一下</p><h1>{session.topic?.word}</h1><p className="topic-intro">{session.topic?.intro}</p><div className="topic-divider"/><p className="topic-description">接下来，我们会为你准备一份研究材料。<br/>不用急着记住每一句，先试着理解它。</p><div className="button-row centered"><button className="secondary" disabled={!!busy} onClick={draw}><Shuffle size={17}/> 换一个词</button><button className="primary" disabled={!!busy} onClick={research}>准备研究材料 <ArrowRight size={17}/></button></div></section>}

      {(session.stage === 'research' || session.stage === 'speech') && <section className="workspace"><div className="workspace-heading"><span className="eyebrow">{sourceName} / {session.stage==='research'?'EXPLORE':'SPEAK'}</span><h1>{session.topic?.word}</h1><p>{session.stage==='research'?'先理解，再用自己的话表达。':'把刚刚理解的知识，讲给一个好奇的朋友听。'}</p></div><div className="reading-layout"><article className="paper">
        ...
        {session.research?.citations && <details className="document-citations"><summary>查看原文依据 · {session.research.source?.name}</summary><p>以下为生成时依据的原文片段。讲解稿依据研究材料编写。</p>{session.research.citations.map(c=><section key={c.chunkId}><h3>{c.label} · 片段 {c.chunkId}</h3><p>{c.text}</p></section>)}<a href={appUrl(`/api/documents/${session.research.source!.id}/download`)}>下载原文件</a></details>}
